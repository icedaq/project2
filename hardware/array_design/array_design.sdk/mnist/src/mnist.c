...
static int CheckData(void)
{
	...
	//Received data: 0
	//Received data: 57E343B7
	// We assume here that the last 7 in the second data package
	// is the 7 that we want. This makes sense because the TDATA
	// BUS gets somehow synthesized to 5 bits only.
	u32 tmp = RxPacket[1];
	result = (tmp >> (0)) & 0xf;
	...
	if (result == testlabel) {
		...
		xil_printf("Result mismatch: %x is not %x!\r\n",
				result, testlabel);
	}
